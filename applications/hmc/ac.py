...
def ac(root, i0, i1, cdims):
    ...
    for x in mf:
        ...
        mf[x] = g.block.transfer(
            mf[x].grid, 
            g.grid(cdims + [i1-i0], g.double), 
            mf[x].otype
        ).sum(mf[x])

        mf[x] = mf[x][0:cdims[0],0:cdims[1],0:cdims[2],0:cdims[3],0:i1-i0].real.reshape(i1-i0, np.prod(cdims))
